...
Файл `.env` обязателен: учётные данные PostgreSQL и `DJANGO_SECRET_KEY` в compose не имеют значений по умолчанию. Также задаются порты (`BACKEND_HOST_PORT`, `FRONTEND_HOST_PORT`, `POSTGRES_HOST_PORT` в режиме dev).
